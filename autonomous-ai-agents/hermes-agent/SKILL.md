...
### Wine + MetaTrader 5 compile troubleshooting
If the user asks to compile an MQL5 `.mq5` from a Linux+Wine setup:
1. Do not keep retrying inaccessible Wine-link paths when backend tools fail repeatedly.
2. First recommended path: use the documented MetaEditor compile command from the user's own shell, not from Hermes backend tools when backend access is blocked.
3. Fallback path: ask for the compiler log and the `.mq5` source text, then apply fixes directly.
4. Common path pitfall: the real MT5 path is usually under the Wine prefix, and symlinks from `/media/...` frequently fail inside tool backends.
